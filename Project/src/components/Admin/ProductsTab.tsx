  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-semibold text-slate-800">Catalog Registry</h2>
          <p className="text-sm text-slate-500">Add products, configure custom nested gift hampers and trigger instant AI SEO optimizations.</p>
        </div>
        ...
        <div className="flex gap-1.5 p-1 bg-slate-100 rounded-lg">
          <button
            ...
            className={`px-3 py-1.5 text-xs font-semibold rounded-md transition ${activeSubTab === 'products' ? 'bg-white text-slate-800 shadow-xs' : 'text-slate-500 hover:text-slate-700'}`}
          >
            <ShoppingBag className="w-4.5 h-4.5 inline mr-1" /> Products
          </button>
          <button
            onClick={() => { setActiveSubTab('categories'); setEditCategory(null); }}
            className={`px-3 py-1.5 text-xs font-semibold rounded-md transition ${activeSubTab === 'categories' ? 'bg-white text-slate-800 shadow-xs' : 'text-slate-500 hover:text-slate-700'}`}
          >
            <Layers className="w-4.5 h-4.5 inline mr-1" /> Categories
          </button>
          <button
            onClick={() => { setActiveSubTab('brands'); setEditBrand(null); }}
            className={`px-3 py-1.5 text-xs font-semibold rounded-md transition ${activeSubTab === 'brands' ? 'bg-white text-slate-800 shadow-xs' : 'text-slate-500 hover:text-slate-700'}`}
          >
            <Sparkles className="w-4.5 h-4.5 inline mr-1" /> Brands
          </button>
        </div>
      </div>
          ...
          {!editProduct ? (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3.5">
                ...
                >
                  <span className={`text-[10px] block font-bold uppercase tracking-widest leading-none mb-1.5 ${stockFilter === 'all' ? 'text-slate-400' : 'text-slate-400'}`}>Total Items</span>
                  <div className="flex justify-between items-baseline">
                    <span className="text-2xl font-black font-mono leading-none">{activeProducts.length}</span>
                    <span className={`text-[10px] font-bold font-mono ${stockFilter === 'all' ? 'text-slate-300' : 'text-slate-500'}`}>Active Catalog</span>
                  </div>
                </button>
                ...
                >
                  <span className={`text-[10px] block font-bold uppercase tracking-widest leading-none mb-1.5 ${stockFilter === 'normal' ? 'text-slate-400' : 'text-slate-400'}`}>Normal Stock</span>
                  <div className="flex justify-between items-baseline">
                    <span className="text-2xl font-black font-mono leading-none">{normalStockCount}</span>
                    <span className="text-[10px] font-bold text-emerald-500 font-mono">In Stock</span>
                  </div>
                </button>
                ...
                >
                  <span className={`text-[10px] block font-bold uppercase tracking-widest leading-none mb-1.5 ${stockFilter === 'out_of_stock' ? 'text-slate-300' : 'text-slate-400'}`}>Out of Stock</span>
                  <div className="flex justify-between items-baseline">
                    <span className={`text-2xl font-black font-mono leading-none ${stockFilter === 'out_of_stock' ? 'text-rose-400' : 'text-rose-600'}`}>{outOfStockCount}</span>
                    <span className="text-[10px] font-bold text-rose-500 font-mono">Sold Out</span>
                  </div>
                </button>
                ...
                <button
                  ...
                  className={`p-4 text-left rounded-xl border transition-all cursor-pointer ${
                    stockFilter === 'out_of_stock_orderable'
                      ? 'bg-slate-900 border-slate-900 text-white shadow-md scale-[1.01]'
                      : 'bg-white border-slate-100 text-slate-700 hover:border-slate-200 hover:shadow-sm'
                  }`}
                >
                  <span className={`text-[10px] block font-bold uppercase tracking-widest leading-none mb-1.5 ${stockFilter === 'out_of_stock_orderable' ? 'text-slate-300' : 'text-slate-400'}`}>Orderable Overrides</span>
                  <div className="flex justify-between items-baseline">
                    <span className={`text-2xl font-black font-mono leading-none ${stockFilter === 'out_of_stock_orderable' ? 'text-violet-400' : 'text-violet-600'}`}>{outOfStockOrderableCount}</span>
                    <span className="text-[10px] font-bold text-violet-500 font-mono">Override ON</span>
                  </div>
                </button>
              </div>
              ...
              {/* Toolbar */}
              <div className="flex flex-col lg:flex-row gap-4 items-center justify-between p-4 bg-white border border-slate-100 rounded-xl">
                <div className="flex flex-wrap gap-2.5 w-full lg:w-4/5 items-center">
                  <div className="min-w-[160px] flex-1">
                    <input
                      ...
                      className="px-3.5 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg w-full focus:outline-none focus:border-rose-300"
                    />
                  </div>
                  <select
                    value={selectedCat}
                    onChange={(e) => setSelectedCat(e.target.value)}
                    className="px-3.5 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none min-w-[130px]"
                  >
                  ...
                  <select
                    ...
                    className="px-3.5 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none min-w-[130px] font-medium"
                  >
                  ...
                  <select
                    ...
                    className="px-3.5 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none min-w-[135px] font-medium"
                  >
                  ...
                  {(selectedCat || stockFilter !== 'all' || compositionFilter !== 'all' || statusFilter !== 'all' || search) && (
                    <button
                      ...
                      className="px-3.5 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 rounded-lg border border-rose-200 cursor-pointer transition shrink-0"
                    >
                      Reset Filters
                    </button>
                  )}
                ...
                <div className="flex flex-wrap gap-2 justify-end w-full lg:w-1/5 shrink-0">
                  <button
                    ...
                    className="px-3.5 py-2 text-xs font-semibold bg-slate-105 hover:bg-slate-200 text-slate-700 rounded-lg border border-slate-200 inline-flex items-center gap-1.5 transition"
                  >
                    <FileOutput className="w-4 h-4" /> Export CSV
                  </button>
                  <label className="px-3.5 py-2 text-xs font-semibold bg-slate-105 hover:bg-slate-200 text-slate-700 rounded-lg border border-slate-200 inline-flex items-center gap-1.5 transition cursor-pointer">
                    <FileInput className="w-4 h-4" /> Import CSV
                  ...
                  <button
                    ...
                    className="px-4 py-2 text-xs font-semibold bg-rose-600 hover:bg-rose-700 text-white rounded-lg inline-flex items-center gap-1.5 shadow-sm transition"
                  >
              ...
              <div className="bg-white border border-slate-100 rounded-xl overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm text-slate-600">
                    <thead className="bg-slate-50 border-b border-slate-100 text-slate-400 font-semibold text-xs uppercase tracking-wider">
                      <tr>
            ...
            /* PRODUCT FORM EDITOR (INCLUDING COBO HAMPER BUILDER) */
            <div className="bg-white border border-slate-100 rounded-xl p-6 space-y-6">
              <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                ...
                <button
                  ...
                  className="px-3.5 py-1.5 text-xs bg-slate-100 text-slate-500 rounded-lg hover:bg-slate-200"
                >
                  Back to List
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Product Title</label>
                    <input
                      type="text"
                      ...
                      className="w-full px-3.5 py-2 border border-slate-200 bg-slate-50 rounded-lg text-sm"
                    />
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Unique SKU</label>
                      <input
                        type="text"
                        value={editProduct.sku || ''}
                        onChange={(e) => setEditProduct({ ...editProduct, sku: e.target.value })}
                        className="w-full px-3.5 py-2 border border-slate-200 bg-slate-50 rounded-lg text-sm font-mono"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Slug Handle</label>
                      <input
                        type="text"
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Original Price (NPR Base)</label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="block text-xs font-bold tracking-wider text-rose-500 uppercase mb-1">Discount Price (NPR)</label>
                      <input
                        type="number"
                        ...
                        className="w-full px-3.5 py-2 border border-rose-200 bg-rose-50/20 text-rose-700 rounded-lg text-sm font-mono placeholder:text-slate-400"
                        placeholder="Normal priced product"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Cost Price (NPR)</label>
                      <input
                        type="number"
                        value={editProduct.costPrice || 0}
                        onChange={(e) => setEditProduct({ ...editProduct, costPrice: parseFloat(e.target.value) || 0 })}
                        className="w-full px-3.5 py-2 border border-slate-200 bg-slate-50 rounded-lg text-sm font-mono"
                        placeholder="Estimating ROI profit margins"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Remaining Stock</label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Low Stock Warning</label>
                      <input
                        type="number"
                  ...
                  <div className="space-y-3.5">
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Publish on Categories</label>
                      <div className="max-h-28 overflow-y-auto p-3.5 border border-slate-200 bg-slate-50 rounded-lg grid grid-cols-2 gap-2 text-left">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Brand Guild</label>
                        <select
                          value={editProduct.brandId || ''}
                          onChange={(e) => setEditProduct({ ...editProduct, brandId: e.target.value })}
                          className="w-full px-3.5 py-2 border border-slate-200 bg-slate-50 rounded-lg text-sm font-semibold text-slate-700"
                        >
                      ...
                      <div className="md:col-span-1">
                        <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Catalog Registry Status</label>
                        <select
                      ...
                      <div className="col-span-1 md:col-span-2">
                        <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">
                          Delivery Availability Groups (Select 1 or more)
                        </label>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 bg-slate-50 border border-slate-200 p-3 rounded-lg max-h-48 overflow-y-auto">
                          {state.deliveryGroups?.map(g => {
                            ...
                            return (
                              <label key={g.id} className="flex items-start gap-2.5 p-2 bg-white rounded-md border border-slate-200 shadow-xs cursor-pointer hover:border-indigo-300 transition select-none">
                                <input
                                  type="checkbox"
                                  checked={isChecked}
                                  onChange={(e) => {
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Product Description</label>
                    <textarea
                      rows={3}
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Product Long Description (SEO Rich Paragraphs / Full Content)</label>
                    <textarea
                      rows={5}
                      value={editProduct.longDescription || ''}
                      onChange={(e) => setEditProduct({ ...editProduct, longDescription: e.target.value })}
                      placeholder="Write comprehensive product details, floristry notes, ingredient highlights, care guidelines, and gifting parameters for optimal Google SEO indexing..."
                      className="w-full px-3.5 py-2 border border-slate-200 bg-slate-50 rounded-lg text-sm"
                    />
                    <p className="text-[10px] text-slate-400 mt-1">
                      This text field allows fully fledged SEO optimization to drive search visibility for individual gifting products.
                    </p>
                  </div>

                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">
                      Additional Note (Displayed on Customer Product Page)
                    </label>
                    <textarea
                      rows={2}
                      value={editProduct.additionalNote || ''}
                      onChange={(e) => setEditProduct({ ...editProduct, additionalNote: e.target.value })}
                      placeholder="e.g. Please order 24 hours in advance for custom design elements or specific preferences."
                      className="w-full px-3.5 py-2 border border-slate-200 bg-slate-50 rounded-lg text-sm font-semibold text-slate-800"
                    />
                    <p className="text-[10px] text-slate-400 mt-1">
                      This dynamic message will display prominently to customers on the product detail page under the delivery table constraints.
                    </p>
                  </div>
                  ...
                  <div className="space-y-3.5 border-t border-slate-100 pt-4">
                    ...
                    <div className="border border-dashed border-slate-200 rounded-xl p-4 bg-slate-50/50 text-center hover:bg-slate-50 hover:border-rose-400 transition relative">
                      <input
                        type="file"
                        accept="image/*"
                            ...
                            <div key={i} className="p-2 border border-slate-150 rounded-xl flex items-center justify-between gap-3 bg-white hover:border-rose-200 transition">
                              ...
                              <div className="flex items-center gap-1.5 shrink-0">
                                ...
                                <select
                                  ...
                                  className="px-2 py-1 border border-slate-200 bg-slate-50 hover:bg-slate-100 rounded-lg text-[10px] font-semibold text-slate-700 cursor-pointer"
                                >
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                <button
                  onClick={() => setEditProduct(null)}
                  className="px-4 py-2 text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-lg"
                >
                  Cancel
                </button>
                <button
              ...
              <div className="bg-slate-50 p-3 rounded-lg border border-slate-200/60 space-y-3 text-left">
                <div className="flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="p-1 px-1.5 text-[8.5px] bg-indigo-50 hover:bg-indigo-100 text-[#492583] rounded font-bold transition"
                  >
                    + Add Placement
                  </button>
                </div>
