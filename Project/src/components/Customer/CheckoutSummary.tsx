...
import { Trash2, Ticket, CheckCircle2, ChevronRight } from 'lucide-react';
...
export default function CheckoutSummary({
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      {/* A. BASKET SUMMARY CARD */}
      <div className="bg-white p-5 rounded-2xl border border-rose-100 shadow-2xs space-y-3.5 text-left">
        <div className="flex items-center justify-between font-bold text-xs uppercase tracking-wide text-left pb-2 border-b border-rose-50/80">
          <span className="flex items-center gap-1.5 text-slate-800 font-bold font-sans">
            📦 Gift Hamper Selection
          </span>
          <span className="text-[10px] font-mono bg-rose-50 text-rose-600 border border-rose-100 px-2 py-0.5 rounded font-bold">
        ...
        <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
          {cartItems.map((item, itemIdx) => {
            ...
            return (
              <div key={item.productId + '-' + itemIdx} className="p-3 bg-rose-50/25 border border-rose-100/50 rounded-xl space-y-2 text-left">
                <div className="flex gap-2.5 justify-between items-center">
                  <img 
                    src={(prod.images && prod.images[0]) || 'https://images.unsplash.com/photo-154946?q=80&w=600&auto=format&fit=crop'} 
                    className="w-10 h-10 rounded object-cover border border-rose-100/60" 
                    alt="" 
                  />
                  <div className="flex-1 min-w-0">
                    <h5 className="font-serif italic truncate text-xs font-bold text-slate-800">{prod.name}</h5>
                    <span className="font-mono text-[9.5px] text-slate-500 font-semibold">{selectedCurrency.symbol} {convertedProdPrice.toLocaleString()} each</span>
                    
                    {item.selectedVariations && item.selectedVariations.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1 justify-start max-w-full">
                        {item.selectedVariations.map((vOpt, vOptIdx) => (
                          <span 
                            key={vOptIdx} 
                            className="inline-block text-[8px] font-mono px-1 py-0.5 rounded bg-white text-rose-800 border border-rose-100 leading-none"
                          >
                  ...
                  <div className="flex gap-1.5 items-center text-[10px]">
                    <div className="flex rounded-md border border-rose-150 bg-white">
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="px-2 py-0.5 text-slate-650 font-bold hover:bg-rose-50 transition border-0 bg-transparent cursor-pointer"
                      >
                        -
                      </button>
                      <span className="px-1.5 py-0.5 font-mono font-bold text-xs text-slate-800 flex items-center justify-center bg-rose-50/40">{item.quantity}</span>
                      <button
                        type="button"
                        onClick={() => {
                          const copy = [...cartItems];
                    ...
                    <button
                      ...
                      className="p-1 hover:bg-red-500/10 text-rose-500 rounded transition cursor-pointer border-0 bg-transparent"
                    >
      ...
      {/* B. PREMIUM ADD-ONS CARD */}
      <div className="bg-white p-5 rounded-2xl border border-rose-100 shadow-2xs space-y-3.5 text-left">
        <span className="text-[10px] uppercase font-bold tracking-widest text-[#a855f7] font-mono block">Premium Add-ons & Wrapping</span>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {(() => {
            ...
            if (activeAndAllowedFees.length === 0) {
              return (
                <div className="text-center py-4 bg-slate-50 border border-slate-100 rounded-xl text-xs font-medium text-slate-500 w-full col-span-full">
                ...
                <div key={f.id} className="w-full space-y-2">
                  <label 
                    className={`flex justify-between items-start gap-2 p-3 rounded-xl border select-none transition ${
                      isChecked 
                        ? 'bg-rose-500/[0.03] border-rose-400 text-rose-700 font-semibold font-sans cursor-pointer' 
                        : 'bg-white border-rose-100 text-slate-700 hover:bg-slate-50 font-sans cursor-pointer'
                    }`}
                  >
                    <div className="flex items-start gap-2.5 flex-1">
                      <input
                        ...
                        className="w-4 h-4 rounded text-rose-600 focus:ring-rose-500 border-rose-200 cursor-pointer mt-0.5"
                      />
                      <div className="flex-1">
                        <span className="text-[11.5px] font-semibold text-slate-800 leading-snug block">{f.name}</span>
                      </div>
                    </div>
                    <span className="font-mono text-[11px] text-rose-600 font-bold shrink-0 ml-2 mt-0.5">
      ...
      {/* C. COUPON FIELD CARD */}
      <div className="bg-white p-5 rounded-2xl border border-rose-100 shadow-2xs space-y-3.5 text-left">
        <span className="text-[10px] uppercase font-bold tracking-widest text-[#a855f7] font-mono block">Apply Promotional Coupon</span>
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Ticket className="w-3.5 h-3.5 text-slate-400 absolute left-3 top-3.5" />
            <input
              type="text"
              placeholder="e.g. KOSELIFEST"
              className="pl-9 pr-2 py-2 text-xs font-mono font-bold border border-rose-150 rounded-xl uppercase w-full focus:outline-none focus:border-rose-455 bg-white text-slate-850"
              value={couponCode}
          ...
          <button
            ...
            className="px-4 bg-[#1e293b] text-white hover:bg-[#0f172a] font-bold tracking-wide rounded-xl transition py-2 cursor-pointer font-mono text-xs border-0"
          >
            Verify
          </button>
        </div>
        {couponFeedback && (
          <div className="text-[10px] font-bold text-rose-500 font-sans mt-1">{couponFeedback}</div>
        )}
      </div>

      {/* D. CURRENCY & WALLET METHOD CARD */}
      <div className="bg-white p-5 rounded-2xl border border-rose-100 shadow-2xs space-y-4 text-left">
        <div className="flex justify-between items-center pb-2 border-b border-rose-50/80">
          <span className="text-[10px] uppercase font-bold tracking-widest text-[#a855f7] font-mono block">Billing Configuration</span>
        </div>

        {/* Currency Selector */}
        <div className="flex justify-between items-center p-2.5 bg-rose-50/20 border border-rose-100 rounded-xl">
          <span className="text-[10.5px] font-semibold text-slate-700">Display Currency</span>
          <div className="flex p-0.5 rounded-lg border bg-white border-rose-150 gap-1">
            {state.currencies.map(curr => {
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-0.5 text-[10px] font-mono font-bold rounded transition-all duration-205 cursor-pointer border-1 ${
                    isSelected
                      ? 'bg-rose-600 text-white font-extrabold shadow-sm border-rose-600'
                      : 'text-slate-500 hover:text-slate-800 hover:bg-rose-50 border-transparent'
                  }`}
                >
                  {curr.code}
                </button>
        ...
        <div className="space-y-2.5">
          <span className="text-[10.5px] font-bold text-slate-705 block">Choose Payment Method</span>
            ...
            {[...(state.paymentGateways || [])]
              ...
              .filter(gw => {
                if (gw.acceptableCurrencies && gw.acceptableCurrencies.length > 0) {
                  ...
                  return upperCurrencies.includes(selectedCurrency.code.toUpperCase());
                }
                if (gw.id === 'esewa' || gw.id === 'khalti' || gw.id.startsWith('fonepay') || gw.id === 'cod') {
                  return selectedCurrency.code.toUpperCase() === 'NPR';
                }
              ...
              .map(gw => {
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-xl border text-center flex flex-col items-center justify-center gap-2.5 transition cursor-pointer hover:border-rose-400 relative overflow-hidden min-h-[105px] ${
                      isSelected
                        ? 'bg-rose-500/[0.04] border-rose-500 text-rose-700 font-bold shadow-xs' 
                        : 'bg-white border-rose-150 text-slate-700'
                    }`}
                  >
                    {isSelected && (
                      <span className="absolute top-0 right-0 bg-rose-600 text-white font-bold text-[8px] font-mono px-1.5 py-0.5 rounded-bl select-none">
                        ✓
                      </span>
                    )}

                    {gw.logoUrl ? (
                      <div className="h-10 w-16 shrink-0 flex items-center justify-center bg-white rounded-lg border border-slate-200 overflow-hidden shadow-2xs">
                        <img
                          src={gw.logoUrl}
                          alt={gw.name}
                          className="h-full w-full object-contain p-0.5"
                          onError={(e) => {
                            (e.target as HTMLImageElement).src = 'https://cdn-icons-png.flaticon.com/512/2897/2897864.png';
                          }}
                        />
                      </div>
                    ) : (
                      <CheckCircle2 className={`w-5 h-5 shrink-0 ${isSelected ? 'text-rose-600' : 'text-slate-455'}`} />
                    )}

                    <div className="min-w-0">
                      <span className="text-[10.5px] block font-bold leading-tight break-words max-w-[125px] line-clamp-2">{gw.name}</span>
                    </div>
                  </button>
                );
              })}
      ...
      {/* E. PRICE CALCULATION SUMMARY CARD & CTA */}
      <div className="bg-[#FFFDFD] p-5 rounded-2xl border border-rose-200 shadow-sm space-y-4 text-left">
        ...
        <div className="space-y-2 font-sans text-xs">
          <div className="flex justify-between text-slate-650">
            ...
            <span className="font-mono text-slate-800">{selectedCurrency.symbol} {subtotalConverted.toLocaleString(undefined, { maximumFractionDigits: 1 })}</span>
          </div>
          ...
          <div className="flex justify-between font-bold text-sm pt-3 border-t border-rose-150 text-slate-850">
            <span>Grand Final Total ({selectedCurrency.code}):</span>
            <span className="font-mono text-rose-600 font-extrabold">{selectedCurrency.symbol} {grandTotalConverted.toLocaleString(undefined, { maximumFractionDigits: 1 })}</span>
          </div>
        </div>

        {selectedCurrency.code !== 'NPR' && (
          <div className="p-2.5 border border-rose-200/50 bg-rose-50/50 text-[10.5px] leading-relaxed text-slate-600 rounded-xl flex items-start gap-1.5 select-none">
            <span className="text-rose-500 text-xs mt-0.5">⚠️</span>
            <span>
              Final settlement will be based on NPR and may vary slightly due to currency conversion by your bank and the bank in Nepal.
            </span>
          </div>
        )}
        ...
        <button
          ...
          type="button"
          className="w-full py-3 bg-gradient-to-r from-rose-600 to-rose-700 hover:from-rose-700 hover:to-rose-800 text-white font-extrabold text-xs uppercase tracking-widest rounded-xl transition shadow-md shadow-rose-500/15 cursor-pointer border-0 mt-2"
        >
          PROCEED TO PAY
        </button>
      </div>
