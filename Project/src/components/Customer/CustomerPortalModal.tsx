...
import { X, CheckCircle2, ShoppingBag, Eye, Lock, ArrowRight, Clock, HelpCircle, Package, Truck, Smile, AlertTriangle, CreditCard, ChevronLeft, Check, User } from 'lucide-react';
...
export default function CustomerPortalModal({ 
  ...
  // Custom Google Authentic Window Simulation states
  const [isGoogleOAuthOpen, setIsGoogleOAuthOpen] = useState(false);
  const [oauthStep, setOauthStep] = useState<'choose' | 'enter-email' | 'loading'>('choose');
  const [oauthCustomEmail, setOauthCustomEmail] = useState('');
  const [oauthCustomName, setOauthCustomName] = useState('');
  const [oauthError, setOauthError] = useState<string | null>(null);

  const ACTIVE_DEVICE_EMAILS = ['dinesh.dineshchalise@gmail.com'];

  const triggerGoogleOAuth = (emailToUse?: string) => {
    setOauthError(null);
    const targetEmail = emailToUse || typedEmail;
    if (targetEmail) {
      setOauthCustomEmail(targetEmail.trim());
      setOauthCustomName(targetEmail.split('@')[0]);
      setOauthStep('enter-email');
    } else {
      setOauthStep('choose');
    }
  ...
  // Secure 4-field guest order tracking portal states
  ...
  const handleSimulatePaymentCompletion = (e: React.FormEvent) => {
    ...
    if (paymentGateway === 'esewa' || paymentGateway === 'khalti') {
      if (!payPhone.trim()) {
        setPayError('A valid mobile number is required.');
        return;
      }
      if (!payPin.trim() || payPin.length < 4) {
        setPayError('PIN must be at least 4 digits.');
        return;
      }
    } else if (paymentGateway === 'nps' || paymentGateway === 'nabil' || paymentGateway?.includes('card')) {
  ...
  const handleGoogleLogin = (emailToUse?: string, nameToUse?: string) => {
    setIsSigningIn(true);
    setTimeout(() => {
      const email = emailToUse || typedEmail || 'customer@gmail.com';
      const name = nameToUse || (email ? email.split('@')[0] : 'Premium Gifter');
      const user = { email, name };
      onUpdateCustomerUser(user);
      localStorage.setItem('google_customer_auth', JSON.stringify(user));
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('google_customer_auth');
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto animate-fade-in">
      {/* Dimmed backdrop dismiss */}
      <div className="absolute inset-0" onClick={onClose}></div>

      <div className="relative w-full max-w-2xl bg-[#FCF9F9] border border-rose-100 rounded-2xl overflow-hidden shadow-2xl text-left text-slate-800 font-sans z-10 my-8">
        
        {/* Modal Top Ribbon Header */}
        <div className="bg-gradient-to-r from-rose-105 via-white to-rose-50 p-5 border-b border-rose-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-rose-500/10 flex items-center justify-center text-rose-600">
              <Smile className="w-4 h-4" />
            </div>
            <div>
              <h3 className="text-sm font-bold text-slate-800 tracking-wide uppercase">👤 Customer Gifting Lounge</h3>
              <p className="text-[10px] text-slate-500 tracking-wide">Secure live order timelines, Google sync, & recipient logs</p>
            </div>
          </div>
          <button 
            type="button"
            onClick={onClose}
            className="p-1.5 px-3 text-slate-500 hover:text-slate-800 bg-rose-500/10 hover:bg-rose-500/20 rounded-lg text-xs leading-none transition-colors border-0 cursor-pointer"
          >
            ✕ Close
          </button>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          {/* VISUALLY SEGREGATED NAVIGATION ACTIONS */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-2">
            {/* REGISTERED USER SECTOR - FOR SIGN IN & ACCOUNTS */}
            <div className="bg-slate-100/90 border border-slate-250 p-1.5 rounded-xl flex flex-col justify-center space-y-1">
              <span className="text-[8px] font-mono font-extrabold uppercase tracking-widest text-rose-600 block pl-1.5 pt-0.5">
              ...
              <button
                ...
                className={`w-full py-2 text-xs font-bold uppercase tracking-wider rounded-lg transition-all text-center cursor-pointer border-0 flex items-center justify-center gap-1.5 ${
                  activeTab === 'signin'
                    ? 'bg-rose-600 text-white shadow-md font-black'
                    : 'text-slate-650 hover:text-slate-900 bg-white/50 hover:bg-white'
                }`}
              >
                <User className="w-3.5 h-3.5 shrink-0" />
                <span>Sign In & Gifting Lounge</span>
              </button>
            </div>

            {/* GUEST QUICK UTILITIES SECTOR - FOR ANONYMOUS OPERATIONS */}
            <div className="bg-purple-50/50 border border-purple-100 p-1.5 rounded-xl flex flex-col justify-center space-y-1">
              <span className="text-[8px] font-mono font-extrabold uppercase tracking-widest text-[#a855f7] block pl-1.5 pt-0.5">
                🔍 Guest Quick Tools
              </span>
              <div className="grid grid-cols-2 gap-1.5">
                <button
                  ...
                  className={`py-2 text-[10.5px] font-bold uppercase tracking-wider rounded-lg transition-all text-center cursor-pointer border-0 flex items-center justify-center gap-1 ${
                    activeTab === 'track'
                      ? 'bg-[#a855f7] text-white shadow-md font-black'
                      : 'text-purple-700 hover:text-purple-950 bg-white/50 hover:bg-white'
                  }`}
                >
                ...
                <button
                  ...
                  className={`py-2 text-[10.5px] font-bold uppercase tracking-wider rounded-lg transition-all text-center cursor-pointer border-0 flex items-center justify-center gap-1 ${
                    activeTab === 'reminder'
                      ? 'bg-[#a855f7] text-white shadow-md font-black'
                      : 'text-purple-705 hover:text-purple-950 bg-white/50 hover:bg-white'
                  }`}
                >
                  <span>📅 Reminders</span>
                </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-5 pt-1">
            ...
            <div className="md:col-span-5 space-y-4 text-left">
              ...
              <div className="space-y-4">
              {!googleUser ? (
                /* Google Authentication Box (Visual Separation) */
                <div className="bg-white border border-slate-200/85 rounded-2xl p-8 flex flex-col items-center justify-center gap-4 shadow-xs text-center">
                  <div className="w-12 h-12 bg-rose-50 rounded-full flex items-center justify-center text-rose-500">
                    <User className="w-6 h-6" />
                  </div>
                  <div className="space-y-1">
                    <h3 className="text-sm font-bold text-slate-800 font-sans tracking-tight">Unlock your Premium Gifter Lounge</h3>
                    <p className="text-[11px] text-slate-450 max-w-xs mx-auto mt-0.5 leading-normal font-semibold">
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto bg-white border border-[#dadce0] hover:bg-[#f8f9fa] shadow-[0_1px_2px_rgba(60,64,67,0.3)] text-[#3c4043] font-sans font-medium text-xs rounded-lg px-6 py-2.5 flex items-center justify-center gap-3 transition-all duration-150 cursor-pointer text-center select-none"
                  >
                    <svg className="w-4.5 h-4.5 shrink-0" viewBox="0 0 24 24">
                      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                      <path fill="#FBB005" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l3.66-2.85z" />
                      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.85c.87-2.6 3.3-4.53 6.16-4.53z" />
                    </svg>
                    <span className="font-bold text-xs tracking-wide uppercase">Connect Account via Google</span>
                  </button>
                </div>
              ) : (
                ...
                <div className="space-y-4 text-left">
                  ...
                  <div className="flex items-center justify-between p-3.5 bg-emerald-500/[0.02] border border-emerald-500/15 rounded-xl">
                    <div className="flex items-center gap-3">
                      <img src="https://cdn-icons-png.flaticon.com/512/300/300221.png" alt="Google" className="w-6 h-6 animate-spin duration-1000" />
                      <div>
                        <div className="text-[11px] font-extrabold text-slate-800 uppercase tracking-wide flex items-center gap-1">
                          {googleUser.name} Gifter-Lounge
                          <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                        </div>
                        <div className="text-[9.5px] text-slate-500 font-mono leading-none">{googleUser.email}</div>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={handleLogout}
                      className="px-2.5 py-1 bg-white hover:bg-rose-500/10 text-slate-500 hover:text-rose-600 border border-slate-200 rounded-md text-[9px] font-extrabold cursor-pointer transition uppercase"
                    >
                      Logout
                    </button>
                  </div>
        ...
        <div className="md:col-span-7 select-none">
          {(activeTab === 'signin' && googleUser && loungeSubTab === 'reminders') || activeTab === 'reminder' ? (
            ...
            /* Lounge Welcome Perks Card instead of empty Tracker */
            <div className="w-full bg-linear-to-b from-white to-rose-50/20 border border-rose-100 rounded-2xl p-6 flex flex-col justify-between shadow-sm relative overflow-hidden text-left space-y-5">
              <div className="absolute top-0 right-0 p-8 opacity-[0.03]">
                <Smile className="w-40 h-40 text-rose-500" />
              </div>
              <div className="space-y-4">
                <div>
                  <span className="text-[9px] font-mono font-extrabold uppercase tracking-widest text-[#a855f7] block mb-1">⭐️ Lounge Perks</span>
                  <h4 className="text-sm font-bold text-slate-800">Why sign into your Gifter Lounge?</h4>
                </div>
                
                <div className="space-y-3.5">
                  <div className="flex gap-3 items-start">
                    <div className="w-5.5 h-5.5 rounded-full bg-[#a855f7]/10 flex items-center justify-center shrink-0 text-[#a855f7] text-xs font-black">
                      ✓
                    </div>
                    <div>
                      <p className="text-xs font-bold text-slate-700 leading-none">Unified Order Histories Logs</p>
                      <p className="text-[10px] text-slate-500 mt-1 leading-normal">
                        Access all your custom order details, downloadable receipts, and live fulfillment statuses securely in one view.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3 items-start">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-700 leading-none">Family Celebrations & Reminders</p>
                      <p className="text-[10px] text-slate-500 mt-1 leading-normal">
                        Register your close circle's birthdays or anniversaries. The portal auto-alerts you 4 days prior with custom curation choices!
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3 items-start">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-700 leading-none">Express Instant Checkout</p>
                      <p className="text-[10px] text-slate-500 mt-1 leading-normal">
                        Auto-populate addresses, phone numbers, and payment preferences so you can send luxury gift hampers in seconds.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="p-3 bg-rose-50/50 border border-rose-100 rounded-xl text-[10px] text-slate-500 leading-relaxed font-semibold">
                🔒 Your personal info and Google credentials are fully encrypted and will never be shared. Feel free to browse or gift.
              </div>
            </div>
          ) : (
      ...
      {/* GOOGLE AUTHENTICATION OVERLAY POPUP SIMULATION */}
      {isGoogleOAuthOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col text-slate-805 animate-in zoom-in-95 duration-200">
            {/* Fake Browser Bar */}
            <div className="bg-slate-100 px-4 py-2.5 flex items-center justify-between border-b border-rose-100 text-[10px] text-slate-500 font-mono">
              <div className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-red-400" />
                <span className="w-2.5 h-2.5 rounded-full bg-yellow-400" />
                <span className="w-2.5 h-2.5 rounded-full bg-green-400" />
              </div>
              <div className="bg-white/85 border border-slate-200/55 rounded px-8 py-0.5 max-w-[200px] truncate select-all">
                https://accounts.google.com
              </div>
              <button 
                onClick={() => setIsGoogleOAuthOpen(false)}
                className="text-slate-400 hover:text-slate-600 font-bold border-0 bg-transparent cursor-pointer p-1"
              >
                ✕
              </button>
            </div>

            {/* Google Authentication Content */}
              ...
              {oauthStep === 'choose' && (
                <div className="space-y-4 text-left font-sans">
                  <div className="text-center space-y-1 pb-2">
                    <h4 className="text-xl font-medium text-slate-900 tracking-tight">Choose an account</h4>
                    <p className="text-xs text-slate-500">to continue to Koseli Xpress Nepal</p>
                  </div>

                  <div className="divide-y divide-slate-100 border-y border-slate-200 mt-2">
                    {/* Active User Preference */}
                    <button
                      type="button"
                      onClick={() => {
                        setOauthCustomEmail('dinesh.dineshchalise@gmail.com');
                        setOauthCustomName('Dinesh Chalise');
                        setOauthStep('loading');
                        setTimeout(() => {
                          handleGoogleLogin('dinesh.dineshchalise@gmail.com', 'Dinesh Chalise');
                          setIsGoogleOAuthOpen(false);
                        }, 1600);
                      }}
                      className="w-full py-3.5 px-1 hover:bg-slate-50/75 flex items-center justify-between transition cursor-pointer text-left border-0"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-sm font-medium text-slate-800">Dinesh Chalise</div>
                          <div className="text-xs text-slate-400 font-mono">dinesh.dineshchalise@gmail.com</div>
                        </div>
                      </div>
                      <span className="text-[10px] text-slate-400 font-medium">Signed In</span>
                    </button>
                  </div>
                </div>
              )}

              {oauthStep === 'enter-email' && (
                <div className="space-y-4 text-left font-sans">
                  <div className="text-center space-y-1 pb-2">
                    <h4 className="text-xl font-medium text-slate-900 tracking-tight">Sign in with Google</h4>
                    <p className="text-xs text-slate-550">to sync your orders</p>
                  </div>

                  {oauthError && (
                    <div className="bg-amber-50 border border-amber-200/80 text-amber-900 rounded-xl p-3.5 space-y-1.5">
                      <div className="flex items-center gap-1.5 font-bold text-xs text-amber-700">
                        <Lock className="w-4 h-4 shrink-0 text-amber-600" />
                        <span>Google Verification Restrained</span>
                      </div>
                      <p className="text-[11px] text-slate-600 leading-relaxed font-medium">
                        Standard Google security requires that the input Gmail matching this session is signed in on this physical device. Please verify your Google account settings or select an active session on your device list.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="space-y-3 pt-1">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        className="w-full text-xs p-3 bg-white border border-slate-200 rounded-lg text-slate-850 focus:outline-none focus:ring-1 focus:ring-rose-500 font-semibold placeholder:text-slate-400"
                        value={oauthCustomEmail}
                        onChange={(e) => {
                          setOauthCustomEmail(e.target.value);
                          setOauthCustomName(e.target.value.split('@')[0]);
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-extrabold uppercase text-slate-400 tracking-wider">Full Name / Profile Name</label>
                      <input
                        type="text"
                  ...
                  <div className="flex gap-2 pt-4">
                    <button
                      ...
                      className="flex-1 py-3 text-xs font-bold text-slate-550 bg-slate-50 hover:bg-slate-100 rounded-lg transition border border-slate-200"
                    >
                      Back
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        if (!oauthCustomEmail) return;
                        const cleanEmail = oauthCustomEmail.trim().toLowerCase();
                        if (!ACTIVE_DEVICE_EMAILS.includes(cleanEmail)) {
                          setOauthError('Device verification error: ' + oauthCustomEmail + ' not signed in.');
                          return;
                        }
                        setOauthError(null);
                        setOauthStep('loading');
                        setTimeout(() => {
                          handleGoogleLogin(oauthCustomEmail, oauthCustomName || oauthCustomEmail.split('@')[0]);
                          setIsGoogleOAuthOpen(false);
                        }, 1600);
                      }}
                      className="flex-1 py-3 text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 rounded-lg transition shadow-md shadow-rose-600/15 cursor-pointer text-center border-0"
                    >
                      Authenticate Sync
                    </button>
                  </div>
              ...
              {oauthStep === 'loading' && (
                <div className="space-y-6 py-6 flex flex-col items-center justify-center">
                  <div className="relative w-12 h-12">
                    <div className="absolute inset-0 rounded-full border-4 border-slate-100 border-t-[#ea4335] border-r-[#4285f4] border-l-[#fbbc05] border-b-[#34a853] animate-spin" />
                  </div>
                  <div className="space-y-1 text-center">
                    <h5 className="text-xs font-bold text-slate-800">One-click Sync Active...</h5>
                    <p className="text-[10.5px] text-slate-450 font-mono tracking-wide">Resolving Google Identity Token securely</p>
                  </div>
                </div>
              )}
