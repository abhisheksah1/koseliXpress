...
export default function App() {
  ...
  const [customerUser, setCustomerUser] = useState<{ email: string; name: string } | null>(() => {
    const saved = localStorage.getItem('google_customer_auth');
    try {
      ...
      .then(data => {
        if (data && data.paymentGateways && data.paymentGateways.length > 0) {
          const merged = freshDb.paymentGateways.map((loc: any) => {
            ...
            const extraSettings = {
              merchantName: existingExtra.merchantName || localExtra.merchantName || '',
              apiUsername: existingExtra.apiUsername || localExtra.apiUsername || '',
              apiPassword: existingExtra.apiPassword || localExtra.apiPassword || ''
            };
            return {
              ...loc,
              ...serv,
  ...
  }, [currentSlug, selectedProductIdDetails, selectedCategoryFilter, activeView]);
  ...
  const primaryColor = dbState.appearance?.primaryColor || '#f59e0b';
  const secondaryColor = dbState.appearance?.secondaryColor || '#a78bfa';
  ...
  const dynamicBgDeep = isLight ? '#FFF8F8' : `rgb(${darkBgR}, ${darkBgG}, ${darkBgB})`;
  ...
  return (
    <div className={`min-h-screen font-sans flex flex-col justify-between transition-colors duration-300 ${
      isLight ? 'bg-[#FFF8F8] text-slate-800 theme-light' : 'text-slate-300 theme-dark'
      ...
      <style>{`
        ...
        .theme-light aside button.bg-amber-500 {
          background-color: var(--primary-theme) !important;
          color: #ffffff !important;
          border-color: var(--primary-theme) !important;
        }
        ...
        /* Footer styling custom */
        .theme-light footer {
          background-color: #ffffff !important;
          border-top: 1px solid rgba(var(--primary-rgb), 0.12) !important;
          color: #475569 !important;
        }
        .theme-light footer span.text-white,
        .theme-light footer h4.text-white,
        .theme-light footer div.text-white {
          color: #0f172a !important;
        }
        .theme-light footer p.text-slate-450 {
          color: #475569 !important;
        }
        .theme-light footer li span {
          color: #1e293b !important;
        }
        .theme-light footer div[class*="bg-[#0a0a0a]"],
        .theme-light footer div[class*="border-white/5"] {
          background-color: #FFF8F8 !important;
          border-color: rgba(var(--primary-rgb), 0.1) !important;
          color: #334155 !important;
        }
      ...
      {activeView === 'admin' ? (
        ...
        <AdminPanel
          ...
          onExitAdmin={() => setActiveView('client')}
        />
      ) : (
        ...
        <>
          <Header
            ...
            onLogoutCustomer={() => {
              ...
              setToastType('info');
              setToastMessage('Logged out of Google account successfully.');
            }}
          />
            ...
            <div className="flex-grow flex items-center justify-center p-6 text-center">
              <div className="max-w-lg w-full bg-[#0d0d0d] border border-white/5 rounded-2xl p-8 space-y-6 shadow-2xl">
                ...
                <div className="pt-6 border-t border-white/5 space-y-2">
                  <p className="text-[10px] text-slate-500 font-mono">Store owner or staff registry?</p>
                  <button
                    onClick={() => setActiveView('admin')}
                    className="px-5 py-2.5 bg-white/5 hover:bg-white/10 text-white border border-white/10 rounded-xl text-xs font-semibold tracking-wider uppercase transition cursor-pointer hover:border-amber-500/30"
                  >
