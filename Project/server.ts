...
// API Store Local JSON Persistence Configuration
const STORE_PATH = path.join(process.cwd(), 'api-store.json');
...
function loadApiStore() {
  try {
    ...
    console.error('Error loading api-store.json:', err);
  }
  ...
  const defaultStore = {
    ...
    manualPaymentSetup: {
      ...
      accountName: 'Koseli Gifting Solutions Pvt. Ltd.',
      accountNumber: '0110017500999',
      qrCode: 'https://cdn-icons-png.flaticon.com/512/10015/10015424.png',
      instructions: 'Please transfer the exact order amount in NPR to our Nabil Bank account or scan the QR code. Send us your bank scroll or confirmation via WhatsApp or support ticket to process dispatch.',
...
function saveApiStore(data: any) {
  try {
    fs.writeFileSync(STORE_PATH, JSON.stringify(data, null, 2), 'utf-8');
  } catch (err) {
    console.error('Error writing api-store.json:', err);
  }
}
...
// Secure eSewa Signature Generator
app.post('/api/payment/generate-esewa-signature', (req, res) => {
  const { amount, transactionUuid, productCode, secretKey } = req.body;
  if (!amount || !transactionUuid || !productCode) {
    return res.status(400).json({ error: 'Missing required parameters: amount, transactionUuid, productCode' });
  }

  try {
    const dataString = `total_amount=${amount},transaction_uuid=${transactionUuid},product_code=${productCode}`;
    const key = secretKey || '8g786g78h76h876h'; // default sandbox credential if empty
    const hash = crypto.createHmac('sha256', key).update(dataString).digest('base64');
    res.json({ signature: hash, msg: dataString });
  } catch (err: any) {
...
// Secure Khalti Payment Initiation Proxy
app.post('/api/payment/initiate-khalti', async (req, res) => {
  const { amount, purchase_order_id, purchase_order_name, return_url, website_url, secretKey } = req.body;
  ...
  try {
    ...
    const authKey = secretKey || 'key_live_secret_val'; // fallback/sandbox key
    
    const payload = {
      return_url: return_url || 'http://localhost:3000',
      website_url: website_url || 'http://localhost:3050',
      amount: Math.round(parseFloat(amount) * 100), // convert Rupees to Paisa
      purchase_order_id: String(purchase_order_id),
      purchase_order_name: purchase_order_name,
      customer_info: {
        name: 'Koseli Customer',
        email: 'info@koselixpress.com',
        phone: '9801234567'
      }
    };
    ...
    res.status(500).json({ error: 'Failed to negotiate with Khalti Payment server' });
  }
});
...
function getNPSSettings() {
  try {
    const db = loadApiStore();
    const gws = db.paymentGateways || [];
    const nps = gws.find((g: any) => g.id === 'nps');
    if (nps) {
      return {
...
// Retrieve backend payment gateways configuration
app.get('/api/payment/get-gateways', (req, res) => {
  try {
    const store = loadApiStore();
    res.json({ paymentGateways: store.paymentGateways || [] });
  } catch (err: any) {
...
app.post('/api/payment/sync-gateways', (req, res) => {
  ...
  try {
    const store = loadApiStore();
    const existingGws = store.paymentGateways || [];

    // Map and merge incoming gateways with existing backend gateways to protect configured credentials
    const mergedGws = paymentGateways.map((incoming: any) => {
      const existing = existingGws.find((e: any) => e.id === incoming.id);
      if (!existing) return incoming;

      // Merge top level fields. If incoming has empty string but existing has value, keep existing
      ...
      const extraSettings = {
        merchantName: incomingExtra.merchantName || existingExtra.merchantName || '',
        apiUsername: incomingExtra.apiUsername || existingExtra.apiUsername || '',
        apiPassword: incomingExtra.apiPassword || existingExtra.apiPassword || ''
      };
...
app.get('/api/integrate/data', (req, res) => {
  const store = loadApiStore();
  res.json(store);
});

// Update integrated API users, logs, and payment settings
app.post('/api/integrate/save', (req, res) => {
...
app.get('/api/v1/products', (req, res) => {
  ...
  const logAndResponse = (status: number, body: any) => {
    ...
    if (db.logs.length > 500) db.logs.pop(); // keep limit
    saveApiStore(db);
...
app.post('/api/v1/orders', async (req, res) => {
  ...
  try {
    const smtpData = fs.readFileSync(path.join(process.cwd(), 'db-state.json'), 'utf8');
    if (smtpData) {
      const rootDb = JSON.parse(smtpData);
      const smtp = rootDb.smtpSettings;
