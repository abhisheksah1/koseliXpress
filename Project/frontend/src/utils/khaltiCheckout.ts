...
export const KHALTI_PENDING_CHECKOUT_KEY = 'koseli_khalti_pending_checkout';

export type PendingKhaltiCheckout = {
  refId: string;
  cartItems: CartItem[];
  senderName: string;
  senderEmail: string;
...
export function savePendingKhaltiCheckout(payload: PendingKhaltiCheckout): void {
  sessionStorage.setItem(KHALTI_PENDING_CHECKOUT_KEY, JSON.stringify(payload));
}

export function loadPendingKhaltiCheckout(): PendingKhaltiCheckout | null {
  try {
    const raw = sessionStorage.getItem(KHALTI_PENDING_CHECKOUT_KEY);
    if (!raw) return null;
    return JSON.parse(raw) as PendingKhaltiCheckout;
  } catch {
    return null;
  }
}
