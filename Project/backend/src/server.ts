...
import {
  ...
  getEnvGatewayDefaults,
  getGatewayById,
  initiateKhaltiPayment,
  lookupKhaltiPayment,
  mergeGatewayConfigs,
  testEsewaConnection,
  testFonepayConnection,
  testKhaltiConnection,
  maskSecret,
  type GatewayConfig,
...
// Secure eSewa Signature Generator (uses server-side credentials)
app.post('/api/payment/generate-esewa-signature', async (req, res) => {
  const { amount, transactionUuid } = req.body;
  if (!amount || !transactionUuid) {
    return res.status(400).json({ error: 'Missing required parameters: amount, transactionUuid' });
  }
  try {
    const gw = await getGatewayById('esewa');
    if (!gw?.merchantId || !gw?.secretKey) {
      return res.status(400).json({ error: 'eSewa not configured. Set credentials in Admin → Payment Gateways or .env' });
    }
    const totalAmount = Number(amount).toFixed(2);
    const payload = await buildEsewaPaymentPayload({
      amount: Number(totalAmount),
      transactionUuid,
      successUrl: req.body.successUrl || `${process.env.APP_URL || 'http://localhost:3000'}/payment/esewa/success`,
      failureUrl: req.body.failureUrl || `${process.env.APP_URL || 'http://localhost:3000'}/payment/esewa/failure`,
      gateway: gw,
    });
    res.json({ signature: payload.fields.signature, productCode: gw.merchantId, msg: payload.fields.signed_field_names });
  } catch (err: any) {
    console.error('eSewa signature failure:', err);
    res.status(500).json({ error: err.message || 'Failed to generate eSewa signature' });
  }
});

// eSewa full payment form payload (redirect checkout)
app.post('/api/payment/initiate-esewa', async (req, res) => {
  const { amount, transactionUuid, successUrl, failureUrl } = req.body;
  if (!amount || !transactionUuid) {
    return res.status(400).json({ error: 'Missing amount or transactionUuid' });
  }
  try {
    const gw = await getGatewayById('esewa');
    if (!gw?.isEnabled) {
      return res.status(400).json({ error: 'eSewa gateway is disabled in admin settings.' });
    }
    const appUrl = process.env.APP_URL || process.env.FRONTEND_URL || 'http://localhost:3000';
    const payload = await buildEsewaPaymentPayload({
      amount: Number(amount),
      transactionUuid,
      successUrl: successUrl || `${appUrl}/payment/esewa/success`,
      failureUrl: failureUrl || `${appUrl}/payment/esewa/failure`,
      gateway: gw,
    });
    res.json(payload);
  } catch (err: any) {
    console.error('eSewa initiate failure:', err);
    res.status(500).json({ error: err.message || 'Failed to prepare eSewa payment' });
  }
});

// Secure Khalti Payment Initiation Proxy (server-side credentials)
app.post('/api/payment/initiate-khalti', async (req, res) => {
  ...
  if (!amount || !purchase_order_id || !purchase_order_name) {
    return res.status(400).json({ error: 'Missing required parameters: amount, purchase_order_id, purchase_order_name' });
  }
  try {
    const gw = await getGatewayById('khalti');
    if (!gw?.isEnabled) {
      return res.status(400).json({ error: 'Khalti gateway is disabled in admin settings.' });
    }
    const data = await initiateKhaltiPayment({
      amount: Number(amount),
      purchaseOrderId: String(purchase_order_id),
    ...
    console.error('Khalti proxy payment failed:', err);
    res.status(500).json({ error: err.message || 'Failed to negotiate with Khalti Payment server' });
  }
});

// Khalti KPG-2 payment verification (lookup API)
app.post('/api/payment/verify-khalti', async (req, res) => {
  const { pidx } = req.body;
  if (!pidx) {
  ...
  try {
    ...
    const lookup = await lookupKhaltiPayment(String(pidx), gw);
    res.json({
      ...lookup,
      verified: lookup.status === 'Completed',
      environment: gw.apiEnvironment === 'live' ? 'live' : 'test',
    });
  } catch (err: any) {
    console.error('Khalti lookup failed:', err);
    res.status(500).json({ error: err.message || 'Khalti payment verification failed' });
  }
});

// Test payment gateway connection (Admin panel)
app.post('/api/payment/test/:gatewayId', async (req, res) => {
  const { gatewayId } = req.params;
  try {
    const gw = await getGatewayById(gatewayId);
    if (!gw) {
      return res.status(404).json({ ok: false, message: `Gateway "${gatewayId}" not found.` });
    }
    let result;
    if (gatewayId === 'esewa') result = await testEsewaConnection(gw);
    else if (gatewayId === 'khalti') result = await testKhaltiConnection(gw);
    else if (gatewayId.startsWith('fonepay')) result = await testFonepayConnection(gw);
    else return res.status(400).json({ ok: false, message: `Testing not supported for gateway "${gatewayId}".` });
    res.json(result);
  } catch (err: any) {
...
// Env credential status (masked) for admin UI
app.get('/api/payment/env-status', (_req, res) => {
...
// ==========================================
// NEPAL PAYMENT SOLUTION (NPS / OnePG) INTEGRATION
// ==========================================

function generateHMACSHA512(value: string, secretKey: string): string {
  return crypto.createHmac('sha512', secretKey).update(value, 'utf-8').digest('hex');
}

function calculateNPSSignature(payload: Record<string, any>, secretKey: string): string {
  // 1. Filter out case-insensitive "signature" and any null/undefined keys
  const sortedKeys = Object.keys(payload)
    .filter(key => key.toLowerCase() !== 'signature' && payload[key] !== undefined && payload[key] !== null)
    .sort();

  // 2. Concatenate values of keys in alphabetical order
  const concatenatedValues = sortedKeys.map(key => String(payload[key])).join('');

  // 3. Generate HMACSHA512 lowercase hex string
  return generateHMACSHA512(concatenatedValues, secretKey);
}

async function getNPSSettings() {
  try {
    const db = await getApiStore();
    const gws = db.paymentGateways || [];
    const nps = gws.find((g: { id?: string }) => g.id === 'nps') as GatewayConfig | undefined;
    if (nps) {
      return {
        merchantId: nps.merchantId || '',
        secretKey: nps.secretKey || '',
        merchantName: nps.extraSettings?.merchantName || '',
        apiUsername: nps.extraSettings?.apiUsername || '',
        apiPassword: nps.extraSettings?.apiPassword || '',
        apiEnvironment: nps.apiEnvironment || 'test',
        isEnabled: nps.isEnabled
      };
    }
  } catch (ex) {
    console.error('Failed to parse paymentGateways from store:', ex);
  }
  return null;
}

// Retrieve backend payment gateways configuration (merged with .env defaults)
app.get('/api/payment/get-gateways', async (_req, res) => {
  try {
    const store = await getApiStore();
    const stored = (store.paymentGateways || []) as import('./services/paymentGatewayService.js').GatewayConfig[];
    const envDefaults = getEnvGatewayDefaults();
    const merged = mergeGatewayConfigs(stored, envDefaults);
    res.json({ paymentGateways: merged });
  } catch (err: any) {
    console.error('Failed to get payment gateways:', err);
    res.status(500).json({ error: err.message || 'Failed to fetch gateways.' });
  }
});

// Synchronize frontend payment gateways with server
app.post('/api/payment/sync-gateways', async (req, res) => {
  const { paymentGateways } = req.body;
  if (!paymentGateways || !Array.isArray(paymentGateways)) {
    return res.status(400).json({ error: 'Missing or invalid paymentGateways payload.' });
  }
  try {
    const store = await getApiStore();
    const existingGws = (store.paymentGateways || []) as import('./services/paymentGatewayService.js').GatewayConfig[];

    const mergedGws = paymentGateways.map((incoming: any) => {
      const existing = existingGws.find((e: any) => e.id === incoming.id);
      if (!existing) return incoming;

      const merchantId = incoming.merchantId || existing.merchantId || '';
      const secretKey = incoming.secretKey || existing.secretKey || '';
      const publicKey = incoming.publicKey || existing.publicKey || '';

      const existingExtra = existing.extraSettings || {};
      const incomingExtra = incoming.extraSettings || {};
      const extraKeys = new Set([...Object.keys(existingExtra), ...Object.keys(incomingExtra)]);
      const extraSettings: Record<string, string> = {};
      for (const key of extraKeys) {
        extraSettings[key] = incomingExtra[key] || existingExtra[key] || '';
      }
    ...
    store.paymentGateways = mergedGws;
    await saveApiStore(store);
    res.json({ success: true, message: 'Payment gateways synced and safely merged on backend.' });
  } catch (err: any) {
    console.error('Failed to sync payment gateways:', err);
    res.status(500).json({ error: err.message || 'Sync failed.' });
  }
});

// Secure NPS (Nepal Payment Solution) Token Generation Proxy
app.post('/api/payment/initiate-nps', async (req, res) => {
  const { amount, merchantTxnId } = req.body;
  if (!amount || !merchantTxnId) {
    return res.status(400).json({ error: 'Missing amount or merchantTxnId.' });
  }

  const nps = await getNPSSettings();
  if (!nps) {
    return res.status(404).json({ error: 'NPS payment gateway configuration not synced in system. Please config in Admin panel.' });
  }

  if (!nps.isEnabled) {
    return res.status(400).json({ error: 'NPS payment gateway is currently disabled in admin dashboard.' });
  }

  const { merchantId, secretKey, merchantName, apiUsername, apiPassword, apiEnvironment } = nps;
  if (!merchantId || !secretKey || !merchantName || !apiUsername || !apiPassword) {
    return res.status(400).json({ error: 'NPS Credentials incomplete! Please specify Merchant Name, Merchant ID, Secret Key, API Username and API Password in Administrative Settings.' });
  }

  try {
    const isProd = apiEnvironment === 'production' || apiEnvironment === 'prod' || apiEnvironment === 'live';
    const postUrl = isProd 
      ? 'https://api.nepalpayment.com/GetProcessId' 
      : 'https://apisandbox.nepalpayment.com/GetProcessId';

    // Format amount strictly with 2 decimal places to match NPS signature & numeric representation
    const formattedAmount = Number(amount).toFixed(2);

    // Signature algorithm uses key order: Amount, MerchantId, MerchantName, MerchantTxnId
    const payload = {
    ...
    const signature = calculateNPSSignature(payload, secretKey);
    const fullPayload = { ...payload, Signature: signature };

    // Authenticate basic token username:password
    const basicCredential = Buffer.from(`${apiUsername}:${apiPassword}`).toString('base64');

    console.log(`[NPS Payment] Contacting: ${postUrl}`);
    console.log(`[NPS Payment] Request Payload:`, JSON.stringify(fullPayload));
    ...
    if (!result.ok) {
      ...
      throw new Error(`NPS server returned status code ${result.status}: ${errorText}`);
    }

    const data: any = await result.json();
    console.log(`[NPS Payment] Response:`, JSON.stringify(data));
    console.log(`[NPS Payment] Response code: ${data.code}, msg: ${data.message}`);
    ...
    console.error('NPS initiation error:', err);
    res.status(500).json({ error: err.message || 'Failed to exchange NPS token' });
  }
});

// Secure NPS (Nepal Payment Solution) Status Verification Check
app.post('/api/payment/check-nps-status', async (req, res) => {
  ...
  const nps = await getNPSSettings();
  if (!nps) {
    return res.status(404).json({ error: 'NPS settings not configured.' });
  }

  const { merchantId, secretKey, merchantName, apiUsername, apiPassword, apiEnvironment } = nps;
  if (!merchantId || !secretKey || !merchantName || !apiUsername || !apiPassword) {
    return res.status(400).json({ error: 'NPS payment credentials not complete.' });
  }
  ...
  try {
    ...
    console.log(`[NPS Status Check] Querying order: ${merchantTxnId}`);
    console.log(`[NPS Status Check] Request Payload:`, JSON.stringify(fullPayload));
    ...
    if (!result.ok) {
      throw new Error(`Status check returned code ${result.status}`);
    }

    const data = await result.json();
    console.log(`[NPS Status Check] Response code:`, data?.code, `Status:`, data?.data?.Status);
    res.json(data);
  } catch (err: any) {
    console.error('NPS transaction status check error:', err);
    res.status(500).json({ error: err.message || 'NPS status lookup malfunctioned.' });
  }
});
