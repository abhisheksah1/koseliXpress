...
/** UAT form POST URL */
function esewaFormUrl(isLive: boolean): string {
  return isLive
    ? 'https://epay.esewa.com.np/api/epay/main/v2/form'
    : 'https://rc-epay.esewa.com.np/api/epay/main/v2/form';
}

/** Status check API — UAT: rc.esewa.com.np | Production: esewa.com.np (per eSewa docs) */
function esewaStatusUrl(isLive: boolean): string {
  return isLive
...
export class EsewaGateway implements PaymentGatewayAdapter {
  ...
  async initializePayment(credentials: GatewayCredentials, input: CreatePaymentDto): Promise<InitializePaymentResult> {
    ...
    const successUrl = input.successUrl || getPaymentCallbackUrl('esewa');
    const failureUrl = input.failureUrl || getPaymentFailureUrl('esewa');

    return {
      paymentId: generatePaymentId(),
      status: PaymentStatus.Processing,
      formAction: esewaFormUrl(isLive),
      formFields: {
        amount: totalAmount,
        tax_amount: '0',
  ...
  async verifyPayment(credentials: GatewayCredentials, input: VerifyPaymentDto): Promise<VerifyPaymentResult> {
    ...
    const productCode = credentials.merchantCode || 'EPAYTEST';
    const isLive = isLiveEnvironment(credentials.environment);
    const url = `${esewaStatusUrl(isLive)}?product_code=${encodeURIComponent(productCode)}&total_amount=${encodeURIComponent(totalAmount)}&transaction_uuid=${encodeURIComponent(transactionUuid)}`;
  ...
  async testConnection(credentials: GatewayCredentials): Promise<TestConnectionResult> {
    ...
    return {
      ok: true,
      message: `eSewa HMAC signature valid (${isLive ? 'LIVE' : 'UAT'}). Form: ${esewaFormUrl(isLive)}`,
      details: {
        productCode: maskSecret(productCode),
        signaturePreview: maskSecret(signature),
        statusApi: esewaStatusUrl(isLive),
      },
    };
  }
}
